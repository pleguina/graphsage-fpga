...
def load_float_model(checkpoint_path, in_channels, out_channels, hidden_channels=24):
...
def run_planetoid(name, args):
    ...
    float_model = load_float_model(float_path, dataset.num_features, dataset.num_classes, hidden_channels=args.hidden)
    scales = activation_scales(float_model, [data])
    weights = weight_scales(float_model)
    model = ReducedGraphSAGEPO2QAT(dataset.num_features, dataset.num_classes, scales, weights, hidden_channels=args.hidden)
